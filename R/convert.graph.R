convert.graph <- function(graph, upper){
	...
	if (cg == "array"){
		...
		if (upper) {
			...
			graph <- t(apply(graph, 3, function(x) return(x[upper.tri(x, diag=TRUE)])))
		} else {
...
### may use f-statistics in library genefilter
### rowFtests, colFtests

getpvals <- function(x, y, workspace, weight, alternative, ...){
	if (weight){
		## Kruskal - Wallis test for pvals for weighted
		return(kruskal.test(x=x, g=y, ...)$p.value)
	} else {
		tab <- table(x, y)
		if (nrow(tab) < 2) return(1)
		expected <- rowSums(tab) %*% t(colSums(tab))/sum(tab)
		if (all(expected > 5) && (nrow(tab) > 2 || ncol(tab) > 2)) {
			return(chisq.test(x=x, y=y)$p.value)
			### stopped here
		} else return(fisher.test(x=x, y=y, alternative=alternative, conf.int = FALSE, workspace=workspace, simulate.p.value=TRUE)$p.value)
	}	
